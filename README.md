...
- <b>Cribl & Splunk-as-Code / Pipeline & Detection Compliance</b>
  ...
  - [Splunk Multi-Node Deployment-as-Code (Ansible + Terraform, Splunk) -- Ansible provisions a real indexer/heavy-forwarder topology (conf files pushed, splunkd restarted), Terraform layers the index and NIST-mapped detections on top; verified end-to-end against live containers, 9,829 real events flowing indexer-side](https://github.com/Larry-Wilkes-CyberCloud/splunk-terraform-detections/tree/main/multi-node)
    - [pipeline-assurance-monitor (Python, Terraform, Cribl, Splunk) -- automated drift-detection check proving the masking pipeline still preserves the exact raw-text evidence the NIST 800-53-mapped Splunk detections (AC-7, AC-6(9)) depend on; sends synthetic canary events through the real, live Cribl pipeline -- not a copy -- and fails loudly, naming the exact control and pattern, if anything drifts. Live-verified end-to-end, including a proof-of-concept demo that deliberately broke a masking rule and confirmed the check caught it (and only it)](https://github.com/Larry-Wilkes-CyberCloud/pipeline-assurance-monitor)
